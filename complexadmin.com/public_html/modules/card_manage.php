...
جستجو بر اساس کد کارت / نام هنرجو : <input type="text" name="search" class="input-t" size="45" /> <input type="submit" value="جستجو" class="input-b" style="text-align:center" />
</form>
</center>
<br /><br />
...
while ($row = $admin->row($sql))
{
	...
	$sql_class = $admin->sql("select * from class where (id = '$row[class_id]')");
	$class = $admin->row($sql_class);
...
if (mysqli_num_rows($sql) == 0)
	echo '<tr><td style="background-color:white; border:1px solid red; height:45px; text-align:center; color:red; " colspan=11>متاسفانه نتیجه ای یافت نشد</td></tr>';
