...
<div class="add-in">
<b class="bx">مدیریت دریافت های کارت آموزشی : </b>
<div class="co">
<?PHP
$tblname = 'card_payment';
$pagCo = 20;
$url = 'cost_payment_manage';
if (isset($_GET['op']))
{
	switch ($_GET['op'])
	{
		...
		case 'edit' :
			if (!isset($_POST['post']))
			{
				...
				<table cellpadding="3">
				  <tr>
					...
					<td>
						<select name="card_id" class="input-s">
						';
						$ex[$editrow['card_id']] = 'selected=selected';
						$s = $admin->sql("select * from card order by code desc");
						while ($r = $admin->row($s))
						{
							echo '<option value="'.$r['id'].'" '.$ex[$r['id']].'>نام : '.$r['name'].'&nbsp;&nbsp;&nbsp;&nbsp; کد کارت : '.$r['code'].'&nbsp;&nbsp;&nbsp;&nbsp; مانده حساب : '.number_format($r['remained']).' تومان</option>';
						}
						echo '
						</select>
					</td>
				  ...
				  <tr>
					<td>تاریخ دریافت : </td>
					<td dir="ltr">';
					list($year , $month , $day) = explode('-' , $editrow['date']);
					echo $admin->full_select(1380 , 1400 , 'year' , $year*1).$admin->full_select(1 , 12 , 'month' , $month*1).$admin->full_select(1 , 31 , 'day' , $day*1).'</td>
				  </tr>
				  <tr>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
			...
			else
			{
				...
				$updsql = $admin->sql("update $tblname set card_id = '$_POST[card_id]' , date = '$date' where id = '$_GET[id]' limit 1");
				if ($updsql)
				{
				...
				$admin->LOG('مدیریت گروه حقوق - ویرایش گروه' , $msg);
			}
		break;
	}
}
...
جستجو بر اساس کد کارت / طرف حساب : <input type="text" name="search" class="input-t" size="45" /> <input type="submit" value="جستجو" class="input-b" style="text-align:center" />
</form>
</center>
<br /><br />
...
{
	$sql = @$admin->sql("select * from $tblname where(code = '$_POST[search]' or name like '%$_POST[search]%')");
}
else
{
...
while ($row = @$admin->row($sql))
{
	...
	$card = $admin->supersql("select * from card where id = '$row[card_id]'");
	
	echo '
  <tr height="25" bgcolor="'.$color.'" class="tblrow">
    <td bgcolor="#d1d1d1" align="center">'.$row['id'].'</td>
	<td align="center">'.$card['code'].'</td>
