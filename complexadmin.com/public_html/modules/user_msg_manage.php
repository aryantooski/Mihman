...
<div class="add-in">
<b class="bx">مدیریت پیغام گروهی : </b>
<div class="co">
<?PHP
$tblname = 'msgs';
$pagCo = 20;
		...
		case 'edit' :
			if (!isset($_POST['post']))
			{
				...
				<table cellpadding="2">
				  <tr>
					<td>توضیحات : </td>
					<td><textarea name="text" class="input-s" style="width:500px; height:200px;">'.$editrow['text'].'</textarea></td>
				  </tr>
				  <tr>
					<td>&nbsp;</td>
			...
			{
				$updsql = $admin->sql("update $tblname set text = '$_POST[text]' where id = '$_GET[id]' limit 1");
				...
				$admin->LOG('مدیریت پیغام عمومی - ویرایش' , $msg);
			}
		break;
	}
}
...
جستجو بر اساس متن : <input type="text" name="search" class="input-t" size="45" /> <input type="submit" value="جستجو" class="input-b" style="text-align:center" />
</form>
</center>
<br /><br />
...
{
	$sql = $admin->sql("select * from $tblname where(text like '%$_POST[search]%')");
}
else
{
...
while ($row = $admin->row($sql))
{
  ...
  <tr height="25" bgcolor="'.$color.'" class="tblrow">
	...
	<td align="center">'.substr($row['text'] , 0 , 250).'</td>
	<td align="center" dir="ltr">'.$row['date'].'</td>
...
if (mysqli_num_rows($sql) == 0)
	echo '<tr><td style="background-color:white; border:1px solid red; height:45px; text-align:center; color:red; " colspan=5>متاسفانه نتیجه ای یافت نشد</td></tr>';
